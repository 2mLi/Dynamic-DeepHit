# in this "proto", we perform feature selection by simply... well, removing each feature while keeping the others
# assuming a machine learning process, we select features with results from the train set (Nanfang + Mengchao)

####################
# retrieve working model
####################
...
# find the optimum mean(c-index) in the original model
training_c = np.max(params['train_results']['c_track'])
training_loss = np.min(params['train_results']['total_loss'])

# let us first remove each continuous variables... 
# cont_list = model_configs['cont_list']

this_c = [training_c]
removed_parameters = ['None']
this_loss = [training_loss]
log_loc = target_dir + '/_log.json'
for cont in model_configs['cont_list']: 

    # import parameters again... 
    
    with open(log_loc) as p: 
        params = json.load(p)
    new_parser = params["new_parser"]
    dataset_info = params["dataset_info"]
    evaluation_info = params["evaluation_info"]
    ...
    # alter parameters
    model_configs["early_stopping"]["mode"] = "ON"


    print('==============================================================================================================')
    print('Removing variable ' + str(cont))
    # create new cont_list replacement
    cont_list = [i for i in model_configs['cont_list'] if not i == cont] # a new cont_list, but without cont
    log_list = [i for i in model_configs['log_transform'] if not i == cont]

    # model training
    print(train_dir)
    (tr_x_dim, tr_x_dim_cont, tr_x_dim_bin), (tr_data, tr_time, tr_label), (tr_mask1, tr_mask2, tr_mask3), (tr_data_mi), (tr_id), tr_feat_list = impt.import_dataset(path = train_dir, bin_list_in = model_configs["bin_list"], cont_list_in = cont_list, log_list = log_list)

    (te_x_dim, te_x_dim_cont, te_x_dim_bin), (te_data, te_time, te_label), (te_mask1, te_mask2, te_mask3), (te_data_mi), (te_id), te_feat_list = impt.import_dataset(path = test_dir[0], bin_list_in = model_configs["bin_list"], cont_list_in = cont_list, log_list = log_list)

    (tea_x_dim, tea_x_dim_cont, tea_x_dim_bin), (tea_data, tea_time, tea_label), (tea_mask1, tea_mask2, tea_mask3), (tea_data_mi), (tea_id), tea_feat_list = impt.import_dataset(path = test_dir[1], bin_list_in = model_configs["bin_list"], cont_list_in = cont_list, log_list = log_list)
    ...
    for itr in range(iteration):
        ...
        if (itr+1)%1000 == 0:        
            ...
            if tmp_valid >  min_valid:
                min_valid = tmp_valid
                saver.save(sess, file_path + "/FS/model")
    ...
    # find max c-index and record it 
    this_c.append(np.max(c_track))
    this_loss.append(np.min(total_loss))
    removed_parameters.append(str(cont))

    # additionally, we plot train set c-indices
    # use 6-month as an example
    pred_time = [12]
    tf.reset_default_graph()
    ...
    eval_path = file_path + "/FS"
    df1.to_csv(eval_path + '/' + str(cont) + '_Train_c_index.csv')
    df2.to_csv(eval_path + '/' + str(cont) + '_Train_Brier.csv')


# record results
res_dir = target_dir + '/FS/'
if not os.path.exists(res_dir):
    os.makedirs(res_dir)
# clear up parameters
removed_parameters = [str(i) for i in removed_parameters]

np.savetxt(res_dir + 'c_index', this_c, delimiter = ",")
np.savetxt(res_dir + 'total_loss', this_loss, delimiter = ",")
np.savetxt(res_dir + 'parameters', removed_parameters, delimiter = ",")
