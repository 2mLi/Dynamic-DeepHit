...
print('Computing Dynamic KMP for Train: ' + train_name + '... ...')
...
fig_dir = target_dir + '/eval/tvKMP/'

if not os.path.exists(fig_dir):
    os.makedirs(fig_dir)

threshold_list = np.empty((len(pred_time), len(eval_time)))
...
for pidx in prange: 
    for eidx in erange: 
        ...
        # check with df1
        if df1.iloc[pidx, eidx] < 0.3 and df2.iloc[pidx, eidx] > 0.8: 
            this_pred_time = pred_time[pidx]
            this_eval_time = eval_time[eidx]
            print('There is no or few event during the following timeframe: [' + str(this_eval_time) + ", " + str(this_eval_time + this_pred_time) + "], therefore no discrimination could be made and no KM plot could be produced in this way. ")
        else: 
            ...
            idx = [i > eval_horizon for i in list(tr_time[:, 0])]
            tr_data_sub = tr_data[idx, :, :]

            # for these patients, their risks
            risk_sub = risk[idx]

            # patient id and time
            tr_time_sub = tr_time[idx, 0]
            tr_label_sub = tr_label[idx, 0]
            ...
            fig_name = 'Landmark_' + str(this_eval_time) + '_Horizon_' + str(this_pred_time) + '_' +train_name +'_KMP.png'
            print('Plotting ' + fig_name)
            pfin.savefig(target_dir + '/eval/tvKMP/' + fig_name)
            del p0
            del p1
            del pfin
            del kmf1
...
print('Computing Dynamic KMP for Test set 1: ' + test1_name + '... ...')
...
for pidx in prange: 
    for eidx in erange: 
        ...
        if df1.iloc[pidx, eidx] < 0.3 and df2.iloc[pidx, eidx] > 0.8: 
            ...
            pfin = p1.get_figure()
            plt.title('Landmark: ' + str(this_eval_time) + ', Horizon: ' + str(this_pred_time) + ', p-value: ' + str('%.3e'%min_p) + ', c-index: ' + str('%.3f'%df1.iloc[pidx, eidx]))
            fig_name = 'Landmark_' + str(this_eval_time) + '_Horizon_' + str(this_pred_time) + '_' +test1_name + '_KMP.png'
            print('Plotting ' + fig_name)
            pfin.savefig(target_dir + '/eval/tvKMP/' + fig_name)
            del p0
            del p1 
...
print('Computing Dynamic KMP for Test set 2: ' + test2_name + '... ...')
...
for pidx in prange: 
    for eidx in erange: 
        ...
        if df1.iloc[pidx, eidx] < 0.3 and df2.iloc[pidx, eidx] > 0.8: 
            ...
            # identify pops who are still susceptible
            eval_horizon = this_eval_time
            idx = [i > eval_horizon for i in list(tea_time[:, 0])]
            tea_data_sub = tea_data[idx, :, :]

            # for these patients, their risks
            risk_sub = risk[idx]

            # patient id and time
            tea_time_sub = tea_time[idx, 0]
            tea_label_sub = tea_label[idx, 0]
            ...
            grp1_data = tea_data_sub[grp1_idx, :, :]
            grp1_time = tea_time_sub[grp1_idx]
            grp1_label = tea_label_sub[grp1_idx]
            # apply a mask
            # for i in range(len(grp1_label)): 
                # grp1_label[i] = 

            grp0_data = tea_data_sub[grp0_idx, :, :]
            grp0_time = tea_time_sub[grp0_idx]
            grp0_label = tea_label_sub[grp0_idx]
            ...
            pfin = p1.get_figure()
            plt.title('Landmark: ' + str(this_eval_time) + ', Horizon: ' + str(this_pred_time) + ', p-value: ' + str('%.3e'%min_p) + ', c-index: ' + str('%.3f'%df1.iloc[pidx, eidx]))
            fig_name = 'Landmark_' + str(this_eval_time) + '_Horizon_' + str(this_pred_time) + '_' +test2_name +'_KMP.png'
            print('Plotting ' + fig_name)
            pfin.savefig(target_dir + '/eval/tvKMP/' + fig_name)
            del p0
            del p1 
